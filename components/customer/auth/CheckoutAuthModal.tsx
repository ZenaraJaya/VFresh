...
export default function CheckoutAuthModal({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (!submitting && step === 'auth') onClose();
    };
  ...
  const loginWith = async (emailValue: string, passwordValue: string) => {
    if (isCustomerDemoEmail(emailValue)) {
      toast.error('The customer demo cannot sign in. Register with your own email.');
      return false;
    }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-end justify-center p-4 sm:items-center">
      <button
        ...
        disabled={submitting || step === 'setup'}
        onClick={() => {
      ...
      <div
        ...
        className="relative max-h-[92vh] w-full max-w-md overflow-y-auto rounded-2xl border border-neutral-200 bg-white shadow-xl dark:border-neutral-800 dark:bg-neutral-900"
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-neutral-200 bg-white px-5 py-4 dark:border-neutral-800 dark:bg-neutral-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              >
                {step === 'setup' ? 'Set payment and address' : 'Sign in to continue'}
              </h2>
            </div>
          </div>
          {step === 'auth' ? (
        ...
        {step === 'setup' ? (
          <div className="space-y-3 px-5 py-5">
            <p className="text-sm text-neutral-600 dark:text-neutral-400">
              Add where we should deliver and how you will pay, then continue
              checkout.
            </p>
            <AccountSetupForm
              defaultName={name}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 px-5 py-5">
            <p className="text-sm text-neutral-600 dark:text-neutral-400">
              {mode === 'register'
                ? 'Create an account, then set your delivery address and payment before checkout.'
